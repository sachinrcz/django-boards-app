...
class TopicPostListView(ListView):
    ...
    def get_queryset(self):
        self.topic = get_object_or_404(Topic, pk= self.kwargs.get('topic_pk'), board__pk = self.kwargs.get('pk'))
...
def board_topics(request, pk):
    ...
    query_set = board.topics.order_by('-last_update').annotate(replies=Count('posts')-1)
